...
\definecolor{lightred}{HTML}{ffc2c2}
  ...
  \textbf{\structure{Morphology}}\\
      ...
      \begin{columns}
        \begin{column}{0.49\textwidth}
          \circled{1} We're loading a 10 segment cell from an \emph{.swc} file. In this file, segments are tagged 1, 2 or 3 which correspond the soma (red), an axon (grey) and a dendrite (blue). Arbor let's us label these regions as follows:
...
\end{minted}
      ...
      \begin{columns}
        \begin{column}{0.49\textwidth}
          \begin{center}\includegraphics[width=0.5\linewidth]{scripts/morph.pdf}\end{center}
        \end{column}
        \begin{column}{0.49\textwidth}
          \begin{center}\includegraphics[width=0.5\linewidth]{scripts/branches.pdf}\end{center}
        \end{column}
      \end{columns}
      
      \begin{columns}
        \begin{column}{0.49\textwidth}
          \circled{3} The two points {\color{black}\circled{-}} and the proximal point of the soma {\color{red}\circled{-}} constrain an interval where we want to place specific dynamics. In Arbor, we can access this region by creating a label for the following region:
\begin{minted}[bgcolor=lightgray,breaklines]{python}
labels['roi'] = '(proximal_interval (sum (location 1 0.8) (location 2 0.3)))'
\end{minted}
          Later, we could tell Arbor to 'paint' Hodgkin–Huxley dynamics on the 'roi' regio:
\begin{minted}[bgcolor=lightgray,breaklines]{python}
cell.paint('roi', 'hh')
\end{minted}
        \end{column}
        \begin{column}{0.49\textwidth}
          \begin{center}\includegraphics[width=0.5\linewidth]{scripts/region.pdf}\end{center}
        \end{column}
      \end{columns}
  ...
  \begin{columns}[onlytextwidth]
    ...
    \begin{column}{.49\linewidth}
      ...
      clear semantics for handling the soma.\\[1.5ex]
    \end{column}
  \end{columns}
